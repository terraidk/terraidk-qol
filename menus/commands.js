...
class CommandsVisualCache {
  ...
  handleCommandCreated(commandName) {
    const existingCommand = this.cachedCommands.find(c => c.name === commandName)
    ...
    if (!existingCommand) {
      ...
      ChatLib.chat(PREFIX + `§a+ Added command "${commandName}" to cache (${this.cachedCommands.length} total)`)
    } else {
      ChatLib.chat(PREFIX + `§e Command "${commandName}" already exists in cache`)
    }
  }
  ...
  handleCommandDeleted(commandName) {
    ...
    this.cachedCommands = this.cachedCommands.filter(c => c.name !== commandName)
    ...
    if (this.cachedCommands.length < initialCount) {
      ...
      ChatLib.chat(PREFIX + `§c- Removed command "${commandName}" from cache (${this.cachedCommands.length} total)`)
    } else {
      ChatLib.chat(PREFIX + `§e Command "${commandName}" was not found in cache`)
    }
  }
  ...
  refreshPlaceholderCommands() {
    ...
    if (placeholders.length > 0) {
      ChatLib.chat(PREFIX + `§e${placeholders.length} placeholder command(s) detected. Consider rescanning to get full data.`)
    }
  }
  ...
  cleanStaleCommands() {
    ...
    this.cachedCommands = this.cachedCommands.filter(cmd => {
      ...
      if (cmd.isPlaceholder && cmd.createdAt && Date.now() - cmd.createdAt > 300000) {
        return false
      }
  ...
  handleMouseScroll(direction) {
    ...
    const maxScroll = Math.max(0, this.filteredCommands.length - maxVisibleItems)
  ...
  scanCurrentPage() {
    ...
    commandSlots.forEach(slotIndex => {
      ...
      if (item && item.getName() !== 'Air') {
        ...
        if (commandData) {
          ...
          const existingCommand = this.cachedCommands.find(c => c.name === commandData.name)

          if (!existingCommand) {
            const possibleRename = this.cachedCommands.find(
              c => c.page === this.currentPage && c.slotIndex === slotIndex && c.name !== commandData.name
            )
    ...
    const deletedCommands = this.cachedCommands.filter(
      c => c.page === this.currentPage && !c.isPlaceholder && !currentPageCommands.has(c.name)
    )
    ...
    if (this.isAutoScanning) {
      ...
      if (renamedCommands > 0 && renamedCommands <= 2) message += ` ↻${renamedCommands} renamed`

      if (deletedCommands.length > 0 && deletedCommands.length <= 2) message += ` -${deletedCommands.length} deleted`
      ...
      if (newCommandsFound > 0) message += ` Found ${newCommandsFound} new commands.`
      if (updatedPlaceholders > 0) message += ` Updated ${updatedPlaceholders} placeholders.`

      if (renamedCommands > 0 && renamedCommands <= 2) message += ` Detected ${renamedCommands} renames.`

      if (deletedCommands.length > 0 && deletedCommands.length <= 2) message += ` Removed ${deletedCommands.length} deleted commands.`
  ...
  continueAutoScan() {
    ...
    ChatLib.chat(PREFIX + `§aAuto-scan complete! Scanned all ${this.totalPages} pages with ${this.cachedCommands.length} total commands.`)
    this.isAutoScanning = false
    ...
    if (!filterText) {
      ...
      this.filteredCommands = this.cachedCommands.filter(
        cmd =>
          ...
          (cmd.descriptions && cmd.descriptions.some(desc => desc.toLowerCase().includes(filter)))
      )
    }
  ...
  renderOverlay() {
    try {
      ...
      // Draw title
      const placeholderCount = this.cachedCommands.filter(c => c.isPlaceholder).length
  ...
  drawCommandsList(panelX, listStartY, panelWidth, availableHeight) {
    ...
    const shouldShowCreateButton = this.filteredCommands.length === 0 && filterText.length > 0
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      try {
        if (cmd.ctItem) {
          ...
          this.drawFallbackIcon(itemX + iconMargin, itemY + (itemHeight - iconSize) / 2, iconSize, cmd)
        }
      } catch (e) {
        if (!cmd.isCreateItem) {
          this.drawFallbackIcon(itemX + iconMargin, itemY + (itemHeight - iconSize) / 2, iconSize, cmd)
      ...
      const hasDescription = cmd.hasDescription && (cmd.description || (cmd.descriptions && cmd.descriptions.length > 0))
      ...
      const nameColor = cmd.isCreateItem ? '§f' : i === this.selectedIndex ? '§a' : isHovered ? '§e' : cmd.isPlaceholder ? '§6' : '§f'
      ...
      if (cmd.isCreateItem) {
        ...
        Renderer.drawStringWithShadow(nameColor + commandName, centerX, itemY + (itemHeight - 8) / 2)
      } else {
        const pageCounter = cmd.page ? `§8[P${cmd.page}]` : ''
        const pageCounterWidth = cmd.page ? Renderer.getStringWidth(`[P${cmd.page}]`) : 0

        const maxCharsForName = Math.floor((availableTextWidth - pageCounterWidth - 10) / 6) - 2
        const displayName = commandName.length > maxCharsForName ? commandName.substring(0, maxCharsForName - 3) + '...' : commandName

        const finalDisplayName = cmd.isPlaceholder ? displayName + ' §8[NEW]' : displayName
        ...
        if (hasDescription) {
          let descriptionText = cmd.description || (cmd.descriptions && cmd.descriptions[0])
          if (descriptionText) {
  ...
  handleMouseClick(mouseX, mouseY, button) {
    ...
    if (
      ...
      if (!this.createButton.isDisabled) {
        ...
        ChatLib.chat(PREFIX + `§cPlease scan all pages before creating new commands!`)
      }
      return true
    }
      ...
      if (mouseX >= itemX && mouseX <= itemX + listWidth && mouseY >= itemY && mouseY <= itemY + itemHeight) {
        if (cmd.isCreateItem) {
          if (button === 0) {
            ...
            const existingCommand = this.cachedCommands.find(c => c.name === currentFilterText)
            if (existingCommand) {
              ChatLib.chat(PREFIX + `§cCommand "${currentFilterText}" already exists!`)
              return true
            }
  ...
  createCommand(commandName) {
    if (commandName && commandName.trim().length > 0) {
      ...
      const existingCommand = this.cachedCommands.find(c => c.name === cleanName)
      if (existingCommand) {
