...
class RegionsVisualCache {
    ...
    scanCurrentPage() {
        ...
        ChatLib.chat(PREFIX + `§a[DEBUG] Page ${this.currentPage} scan complete. Found ${newRegionsFound} new regions. Total: ${this.cachedRegions.length}`);
    ...
    updateFilteredRegions() {
        if (!this.filterText) {
            ...
            this.filteredRegions = this.cachedRegions.filter(region => 
    ...
    drawRegionsList(panelX, listStartY, panelWidth, availableHeight) {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            const isHovered = mouseX >= itemX && mouseX <= itemX + listWidth &&
