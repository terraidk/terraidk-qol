...
class RegionsVisualCache {
  ...
  handleRegionCreated(regionName) {
    const existingRegion = this.cachedRegions.find(r => r.name === regionName)
    ...
    if (!existingRegion) {
      ...
      ChatLib.chat(PREFIX + `§a+ Added region "${regionName}" to cache (${this.cachedRegions.length} total)`)
    } else {
      ChatLib.chat(PREFIX + `§e Region "${regionName}" already exists in cache`)
    }
  }
  ...
  handleRegionDeleted(regionName) {
    ...
    this.cachedRegions = this.cachedRegions.filter(r => r.name !== regionName)
    ...
    if (this.cachedRegions.length < initialCount) {
      ...
      ChatLib.chat(PREFIX + `§c- Removed region "${regionName}" from cache (${this.cachedRegions.length} total)`)
    } else {
  ...
  handleRegionRenamed(oldName, newName) {
    ...
    if (region) {
      ...
      ChatLib.chat(PREFIX + `§6Renamed region "${oldName}" → "${newName}" in cache`)
    } else {
      this.handleRegionCreated(newName)
      ChatLib.chat(PREFIX + `§e Region "${oldName}" not found in cache, created placeholder for "${newName}"`)
    }
  }
  ...
  refreshPlaceholderRegions() {
    ...
    if (placeholders.length > 0) {
      ChatLib.chat(PREFIX + `§e${placeholders.length} placeholder region(s) detected. Consider rescanning to get full data.`)
    }
  }
  ...
  cleanStaleRegions() {
    ...
    this.cachedRegions = this.cachedRegions.filter(region => {
      ...
      if (region.isPlaceholder && region.createdAt && Date.now() - region.createdAt > 300000) {
        return false
      }
  ...
  scanCurrentPage() {
        ...
        if (regionData) {
          ...
          if (!existingRegion) {
            const possibleRename = this.cachedRegions.find(
              r => r.page === this.currentPage && r.slotIndex === slotIndex && r.name !== regionData.name
            )

            if (possibleRename) {
              if (renamedRegions < 2) {
                ChatLib.chat(PREFIX + `§6Detected rename: "${possibleRename.name}" → "${regionData.name}"`)
              }
    ...
    const deletedRegions = this.cachedRegions.filter(
      r => r.page === this.currentPage && !r.isPlaceholder && !currentPageRegions.has(r.name)
    )
    ...
    if (this.isAutoScanning) {
      ...
      if (renamedRegions > 0 && renamedRegions <= 2) message += ` ↻${renamedRegions} renamed`

      if (deletedRegions.length > 0 && deletedRegions.length <= 2) message += ` -${deletedRegions.length} deleted`
      ...
      if (newRegionsFound > 0) message += ` Found ${newRegionsFound} new regions.`
      if (updatedPlaceholders > 0) message += ` Updated ${updatedPlaceholders} placeholders.`

      if (renamedRegions > 0 && renamedRegions <= 2) message += ` Detected ${renamedRegions} renames.`

      if (deletedRegions.length > 0 && deletedRegions.length <= 2) message += ` Removed ${deletedRegions.length} deleted regions.`
  ...
  continueAutoScan() {
    ...
    ChatLib.chat(PREFIX + `§aAuto-scan complete! Scanned all ${this.totalPages} pages with ${this.cachedRegions.length} total regions.`)
    this.isAutoScanning = false
  ...
  renderOverlay() {
    try {
      ...
      const placeholderCount = this.cachedRegions.filter(r => r.isPlaceholder).length
  ...
  drawRegionsList(panelX, listStartY, panelWidth, availableHeight) {
    ...
    // No results case
    const shouldShowCreateButton = this.filteredRegions.length === 0 && filterText.length > 0
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      try {
        if (region.ctItem) {
          ...
          this.drawFallbackIcon(itemX + iconMargin, itemY + (itemHeight - iconSize) / 2, iconSize, region)
        }
      } catch (e) {
        if (!region.isCreateItem) {
          this.drawFallbackIcon(itemX + iconMargin, itemY + (itemHeight - iconSize) / 2, iconSize, region)
      ...
      const hasCoords = region.hasCoords && region.from && region.to && region.from !== 'Unknown' && region.to !== 'Unknown'
      ...
      const nameColor = region.isCreateItem ? '§f' : i === this.selectedIndex ? '§a' : isHovered ? '§e' : region.isPlaceholder ? '§6' : '§f'
      ...
      if (region.isCreateItem) {
        ...
        Renderer.drawStringWithShadow(nameColor + regionName, centerX, itemY + (itemHeight - 8) / 2)
      } else {
        const pageCounter = region.page ? `§8[P${region.page}]` : ''
        const pageCounterWidth = region.page ? Renderer.getStringWidth(`[P${region.page}]`) : 0

        // Calculate available space for region name
        const maxCharsForName = Math.floor((availableTextWidth - pageCounterWidth - 10) / 6) - 2
        const displayName = regionName.length > maxCharsForName ? regionName.substring(0, maxCharsForName - 3) + '...' : regionName

        const finalDisplayName = region.isPlaceholder ? displayName + ' §8[NEW]' : displayName
        ...
        if (hasCoords) {
          ...
          const finalCoordText = coordText.length > maxCoordLength ? coordText.substring(0, maxCoordLength - 3) + '...' : coordText
  ...
  handleMouseClick(mouseX, mouseY, button) {
    ...
    if (
      ...
      if (!this.createButton.isDisabled) {
        ...
        ChatLib.chat(PREFIX + `§cPlease scan all pages before creating new regions!`)
      }
      return true
    }
      ...
      if (mouseX >= itemX && mouseX <= itemX + listWidth && mouseY >= itemY && mouseY <= itemY + itemHeight) {
        if (region.isCreateItem) {
          if (button === 0) {
            ...
            const existingRegion = this.cachedRegions.find(r => r.name === currentFilterText)
            if (existingRegion) {
              ChatLib.chat(PREFIX + `§cRegion "${currentFilterText}" already exists!`)
              return true
            }
    ...
    if (mouseX >= panelX && mouseX <= panelX + panelWidth && mouseY >= panelY && mouseY <= panelY + panelDims.height) {
      return true
    }
  ...
  createRegion(regionName) {
    if (regionName && regionName.trim().length > 0) {
      ...
      const existingRegion = this.cachedRegions.find(r => r.name === cleanName)
      if (existingRegion) {
