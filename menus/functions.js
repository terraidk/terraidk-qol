...
class FunctionsCache extends BaseInventoryCache {
    constructor() {
        super({
            title: "Functions",
            inventoryPattern: /^\(\d+\/\d+\) Functions$|^Functions$/,
            slotIndices: [
            ...
            chatPatterns: {
                created: "Created function ${functionName}!",
                deleted: "Deleted the function ${functionName}",
                renamed: "Renamed function ${oldName} to ${newName}",
            },
            dropdownOptions: [
    ...
    handleItemCreated(functionName) {
        const existingFunction = this.state.cachedItems.find(
        ...
        if (!existingFunction) {
            ...
            this.state.cachedItems.push(newFunction);
            this.updateFilteredItems();
            ChatLib.chat(
                PREFIX +
                    `§a+ Added function "${functionName}" to cache (${this.state.cachedItems.length} total)`
            );
        } else {
            ChatLib.chat(
    ...
    handleItemDeleted(functionName) {
        ...
        if (this.state.cachedItems.length < initialCount) {
            ...
            ChatLib.chat(
                PREFIX +
                    `§c- Removed function "${functionName}" from cache (${this.state.cachedItems.length} total)`
            );
        } else {
            ChatLib.chat(
    ...
    handleItemRenamed(oldName, newName) {
        const func = this.state.cachedItems.find((r) => r.name === oldName);

        if (func) {
            func.name = newName;
            func.displayName = `§f${newName}`;
            this.updateFilteredItems();

            ChatLib.chat(
                PREFIX +
                    `§6Renamed function "${oldName}" → "${newName}" in cache`
            );
        } else {
            this.handleItemCreated(newName);
            ChatLib.chat(
                PREFIX +
                    `§e Function "${oldName}" not found in cache, created placeholder for "${newName}"`
            );
        }
    }
    ...
    // Override the renderListItem method for custom function rendering
    renderListItem(
        func,
        itemX,
        itemY,
        ...
        // Draw function icon if available
        try {
            if (func.ctItem) {
...
export const functionsCache = new FunctionsCache();
