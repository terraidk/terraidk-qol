  ...
  open() {
    ...
    gui.registerDraw(() => {
      ...
      this.buttons.forEach((button, index) => {
        ...
        button.hovered = mouseX >= buttonX && mouseX <= buttonX + buttonWidth && mouseY >= buttonY && mouseY <= buttonY + buttonHeight
    ...
    gui.registerClicked((mouseX, mouseY, button) => {
      ...
      const insideMenu = mouseX >= menuX && mouseX <= menuX + menuWidth && mouseY >= menuY && mouseY <= menuY + menuHeight
    ...
    gui.registerKeyTyped((char, keyCode) => {
      if (keyCode === 1) {
        ...
        this.selectedButton = newIndex < 0 ? this.buttons.length - 1 : newIndex
      } else if (keyCode === 208) {
        // Down arrow
        const newIndex = this.selectedButton + this.columns
        this.selectedButton = newIndex >= this.buttons.length ? 0 : newIndex
      } else if (keyCode === 203) {
        // Left arrow
        this.selectedButton = this.selectedButton <= 0 ? this.buttons.length - 1 : this.selectedButton - 1
      } else if (keyCode === 205) {
        // Right arrow
        this.selectedButton = this.selectedButton >= this.buttons.length - 1 ? 0 : this.selectedButton + 1
      } else if (keyCode === 28) {
        // Enter
...
register('tick', () => {
  ...
  if (currentState && !prevGridMenuState) {
    if (currentGui && (currentGui instanceof Java.type('net.minecraft.client.gui.GuiChat') || currentGui.toString().includes('GuiChat'))) {
