...
register("command", (...args) => {
    ...
    if (args.length === 0) {
        ...
        ChatLib.chat("&7&o/reminders [delete <#> | edit <#> <new message/time> | clearall]");
        World.playSound("note.pling", 1, 2);
        return;
    }
    ...
    if (sub === "edit" && args[1] && args.length > 2) {
        ...
        if (timeMs !== null && newMsg.length > 0) {
            ChatLib.chat(PREFIX + `&aEdited reminder #${idx + 1}: &e${newMsg} &7(in ${formatDuration(timeMs)})`);
        } else if (timeMs !== null) {
            ChatLib.chat(PREFIX + `&aEdited reminder #${idx + 1} time: &7(in ${formatDuration(timeMs)})`);
    ...
    if (sub === "clearall") {
        ...
        if (now - lastClearAllAttempt < 10000) {
            ...
            ChatLib.chat(PREFIX + "&6Run &b/reminders clearall &6again within 10 seconds to confirm.");
