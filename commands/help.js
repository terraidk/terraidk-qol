...
function getEnabledCommands() {
  ...
  if (config.enablePartyTransfer)
    commands.push("&a/pt <player> &7- Transfer party to player");
  ...
  if (config.enableParkourCheckpoint)
    commands.push("&a/pcp &7- Go to parkour checkpoint");
  ...
  if (config.enableReminderList)
    commands.push("&a/reminders &7- View/edit/delete reminders");
...
register("command", (...args) => {
  ...
  if (args.length === 0) {
    ChatLib.chat(
      PREFIX +
        "&cUsage: /tqol help [filter] [page] &7or &b/tqol <minecraft command>"
    );
    ChatLib.chat(PREFIX + "&7Examples: &b/tqol help party 1 &7or &b/tqol pcp");
    ChatLib.chat(
      PREFIX + "&7Config: &b/tqol config &7to toggle commands on/off"
    );
    return;
  }
  ...
  ChatLib.chat(
    `&r                 &3Commands &7(Page &e${pageNum}&7 of &6${maxPage}&7)`
  );
