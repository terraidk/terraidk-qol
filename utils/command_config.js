...
const CONFIG_FILE =
  "config/ChatTriggers/modules/terraidk-qol/command_config.json";
...
class Config {
  ...
  get enablePartyTransfer() {
    return this.get("enablePartyTransfer");
  }
  get enablePartyDisband() {
    return this.get("enablePartyDisband");
  }
  get enableLobbyHousing() {
    return this.get("enableLobbyHousing");
  }
  get enableParkourCheckpoint() {
    return this.get("enableParkourCheckpoint");
  }
  get enablePartyWarp() {
    return this.get("enablePartyWarp");
  }
  get enableVariables() {
    return this.get("enableVariables");
  }
  get enableSelfVariables() {
    return this.get("enableSelfVariables");
  }
  get enableReminders() {
    return this.get("enableReminders");
  }
  get enableReminderList() {
    return this.get("enableReminderList");
  }
  get enableFunc() {
    return this.get("enableFunc");
  }
  get enableFuncAliases() {
    return this.get("enableFuncAliases");
  }
  get enableRegion() {
    return this.get("enableRegion");
  }
  get enableRegionAliases() {
    return this.get("enableRegionAliases");
  }
  get enableCommand() {
    return this.get("enableCommand");
  }
  get enableCommandAliases() {
    return this.get("enableCommandAliases");
  }
  get enableMenu() {
    return this.get("enableMenu");
  }
  get enableMenuAliases() {
    return this.get("enableMenuAliases");
  }
  ...
  showConfig() {
    const commands = [
      ...
      {
        key: "enableFuncAliases",
        name: "Function Aliases (/fc, /fr, /fe, /fd)",
      },
      ...
      {
        key: "enableCommandAliases",
        name: "Command Aliases (/cc, /ce, /ca, /cd)",
      },
    ...
    ChatLib.chat(
      "&8&m----------------&r " +
        "&9[&aterraidk's QoL Config&r&9]&r " +
        "&8&m----------------"
    );
  ...
  handleConfigCommand(args) {
    ...
    if (args[0] === "toggle" && args[1]) {
      ...
      if (num >= 1 && num <= commands.length) {
        ...
        ChatLib.chat(
          PREFIX + "&b/ct reload &crequired for changes to take effect!"
        );
      } else {
        World.playSound("mob.endermen.portal", 1, 0.5);
        ChatLib.chat(
          PREFIX + "&cInvalid command number! Use 1-" + commands.length
        );
      }
...
export function registerToggledCommand(configKey, handler, name, aliases) {
  ...
  let cmd = register("command", handler).setName(
    name,
